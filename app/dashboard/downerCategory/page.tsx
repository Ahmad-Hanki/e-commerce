...
import getDownerCategories from "./_actions/getDownerCategories";

const CategoryPage = async () => {
  const catagories = await getDownerCategories();
      ...
      <Container>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-5 gap-12 sm:gap-2">
          <Heading
            ...
            description={"Toplam alt Kategoriler"}
            title={"alt Kategori Veri"}
          />

          <Link href={"/dashboard/downerCategory/add"}>
        ...
        <DataTable
          ...
          filtered="name"
        />
        <Heading title="API" description="API calls for entered Upper Category" />
        <Separator />
        <ApiAlert />
      </Container>
    </div>
